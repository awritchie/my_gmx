...
#define NPA asize(pa)
    ...
    int iframes = 0;
    std::vector<t_water> water (100000);
    data.nhb = 0;
    int nhb = 0;
    read_first_frame(oenv, &status, traj_file, &fr, flags);
/*
 * doesn't work because the variables fr, oenv, and status are updated
 * every frame
 */
//#pragma omp parallel reduction(+ : nhb)
    do {
        int i = fr.time / 4;
        int frame_hbond = 0; // per frame number of hbonds, 0 or 1
        gmx_bool hbond = false;
        water[i] = analyze_frame(&fr, &top, cutoffs);
...
//#pragma omp parallel for num_threads(8) reduction(+ : nhb)
    ...
    double nframes = iframes;
    double percent = data.nhb/nframes * 100.;
    
    std::vector<int> nps(iframes,0);
    std::vector<int> tnps(iframes,0);
    std::vector<int> nhbs(iframes,0);
    std::vector<int> tnhbs(iframes,0);
