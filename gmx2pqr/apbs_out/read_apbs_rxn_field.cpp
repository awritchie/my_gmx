...
int main(int argc, const char * argv[])
{
    ...
    {
        std::cerr << "\nUsage: <pqr> <apbs atom potentials in solvent> <opt: apbs atom potentials in vacuum>\n" << std::endl;
        std::exit(1);
    }
    ...
    if (file.is_open())
    {
        while (file.good())
        {
            ...
            if (not pqrline.empty())
            {
                ...
                else if (std::strncmp(atom.c_str(),"DUM",4) == 0)
                {
                    coordArray.push_back(coords);
                    DumLineNumbers.push_back(n);
                    AlongBV.push_back(n);
    ...
    print_selection(AlongBV, "Potential along bond vector");
    ...
    parse_selection(rxnpot, coordArray, AlongBV);
...
double parse_selection( const std::vector<float> &rxnpot, const std::vector<std::vector<float> > &coord, const std::vector<int> &selection ) 
{
    ...
    if (n>0) 
    {
        ...
        double dV = rxnpot[n1] - rxnpot[n0]; // y1-y0/dx = slope
